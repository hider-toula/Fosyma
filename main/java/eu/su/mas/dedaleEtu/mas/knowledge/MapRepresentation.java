...
public class MapRepresentation implements Serializable {
	...
	public List<String> getnodeAdjacent(Node node){
		
		List<String>nodeadj=new ArrayList<String>();
		
		Iterator<Edge> iterE=node.edges().iterator();
		while (iterE.hasNext()){
			Edge e=iterE.next();
			nodeadj.add(e.getOpposite(node).getId());
	...
	public List<String> getLongestPathToClosestOpenNode(String myPosition) {
		//1) Get all openNodes
		List<String> opennodes=getOpenNodes();
		...
		Optional<Couple<String,Integer>> closest=lc.stream().max(Comparator.comparing(Couple::getRight));
		//3) Compute shorterPath
	...
	public <T extends Node> Iterable<? extends T> getEachNode() {
		return ((MapRepresentation) g).getEachNode();
	}
	...
	public int getNemberOfLinks(String position) {
		
		return this.g.getNode(position).getDegree();
	}
	
	public int getNbEdges() {
		return this.nbEdges;
	}
	public void setNbEdges() {
		 this.nbEdges++;
	}
	...
	public Object clone() {
		...
		for (SerializableNode<String, MapAttribute> n :this.getSerializableGraph().getAllNodes()) {
			
			res.addNode(n.getNodeId(), n.getNodeContent());
			
			}
		
		
		for (SerializableNode<String, MapAttribute> n1 :this.getSerializableGraph().getAllNodes()) {

			if (res.getAllNodes().contains(n1)) {

				for (String s : this.getSerializableGraph().getEdges(n1.getNodeId())) {

					addEdge(res, n1.getNodeId(), s);
	...
	public synchronized void addEdge(SerializableSimpleGraph<String, MapAttribute> res, String idNode1,
		...
		try {
			...
			res.addEdge(((Integer) this.getNbEdges()).toString(), idNode1, idNode2);
